...
public class P1_Simple 
{
	...
	public static void main(String[] args) throws InvalidParameterSpecException, SecurityLevelException, IOException
	{
		...
		for(int newn = 0;newn<11;newn++)
		{
			...
			GroupElementPlaintext decryptedData;
			// Send the Public Key
			...
			ElGamalOnGroupElementCiphertext afterPolynomManipolation;
			for (int i=0; i<n;i++)
			{
				try {
					afterPolynomManipolation = (ElGamalOnGroupElementCiphertext) elGamal
							.reconstructCiphertext((AsymmetricCiphertextSendableData) channel
									.receive());
					decryptedData = (GroupElementPlaintext)elGamal
							.decrypt((AsymmetricCiphertext) afterPolynomManipolation);
					commcomp++;
					for(int j=0;j<n;j++)
					{
						if(decryptedData.equals(check[j]))
						{
