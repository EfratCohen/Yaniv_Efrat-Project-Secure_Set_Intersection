...
public class P2 
{
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InvalidKeyException
	{
		...
		for(int newn = 0;newn<11;newn++)
		{
			...
			for (int i=0;i<n;i++) 
			{
				Bins = hash.returnBalanced(SetY[i]);
				for(int b=0;b<2;b++)//for balanced hash we go over 2 choices of bins.
				{
					for(int j=0;j<Max;j++)
					{
						//enc(ai)^y^i		//we do it all for bin0 and bin1
						c1[j]=dlog.exponentiate(polys[Bins[b]][j].getC1(), BigInteger.valueOf(SetY[i]).pow(j));
						c2[j]=dlog.exponentiate(polys[Bins[b]][j].getC2(), BigInteger.valueOf(SetY[i]).pow(j));
					}
